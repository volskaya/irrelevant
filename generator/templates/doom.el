...
;;
(defgroup {{name}}-theme nil
  "Options for doom-themes"
  :group 'doom-themes)

(defcustom {{name}}-brighter-modeline nil
  "If non-nil, more vivid colors will be used to style the mode-line."
  :group '{{name}}-theme
  :type 'boolean)

(defcustom {{name}}-brighter-comments nil
  "If non-nil, comments will be highlighted in more vivid colors."
  :group '{{name}}-theme
  :type 'boolean)

(defcustom {{name}}-comment-bg {{name}}-brighter-comments
  "If non-nil, comments will have a subtle, darker background. Enhancing their
legibility."
  :group '{{name}}-theme
  :type 'boolean)

(defcustom {{name}}-padded-modeline nil
...
(def-doom-theme {{name}}
  ...
  ((bg         '("{{background}}" nil       nil            ))
   (bg-dark    '("{{background}}" nil       nil            ))
   (bg-alt     '("{{background}}" nil       nil            ))
   (bg-so      '("{{surface}}" nil       nil            ))
   (bg-alt-so  '("{{background}}" nil       nil            ))
   (fg         '("{{foreground}}" "{{foreground}}" "white"        ))
   (fg-alt     '("{{activeForeground}}" "{{activeForeground}}" "white"        ))
   ...
   (grey       "{{inactiveForeground}}")
   (red        '("{{red}}" "#525252" "red"          )) ;
   (orange     '("{{numberColor}}" "#6e43e5" "brightred"    )) ; Numbers
   (green      '("{{stringColor}}" "#6e43e5" "green"        )) ; Strings, brackets
   (teal       '("{{objectColor}}" "#6e43e5" "brightgreen"  )) ; Only hydra and rainbow delimiters
   (yellow     '("{{objectColor}}" "#6e43e5" "yellow"       )) ; Objects, types, Lisp functions
   (blue       '("{{inactiveForeground}}" "#525252" "brightblue"   )) ; brackets, UI colors, autocomplete foreground, cursor
   (dark-blue  '("{{inactiveForeground}}" "#525252" "blue"         )) ; Selection in menus and autocomplete and
   (magenta    '("{{foreground}}" "#ffffff" "magenta"      )) ; Functions and defines
   (violet     '("{{inactiveForeground}}" "#525252" "brightmagenta")) ; this, backdrop UI colors, comment points
   (cyan       '("{{activeForeground}}" "#979797" "brightcyan"   )) ; html.. tags
   (dark-cyan  '("{{activeForeground}}" "#979797" "cyan"         )) ; Was used for blending comments with base4 ba
   ...
   ;; custom categories
   (-modeline-bright {{name}}-brighter-modeline)
   (-modeline-pad
    (when {{name}}-padded-modeline
      (if (integerp {{name}}-padded-modeline) {{name}}-padded-modeline 4)))
